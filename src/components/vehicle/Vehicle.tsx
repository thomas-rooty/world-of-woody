...
import { useControls } from './hooks/useControls.tsx'
import { useVehicleController } from './hooks/useVehiculeController.tsx'
...
function Tractor() {
  ...
  const { stiffnessFront, stiffnessBack, restFront, restBack, travelFront, travelBack } = useLeva({
    front: folder({
      stiffnessFront: {
        label: 'stiffness',
        value: 30,
  ...
  }, [restBack, restFront, stiffnessBack, stiffnessFront, travelBack, travelFront, vehicleController])
  ...
  useFrame(({ clock }) => {
    ...
    const engineForce = Number(controls.accelerate) * accelerateForce - Number(controls.brake) * brakeForce

    vehicleController.setWheelEngineForce(0, engineForce)
    vehicleController.setWheelEngineForce(1, engineForce)

    const currentSteering = vehicleController.wheelSteering(0) || 0
    const steerDirection = Number(controls.steerLeft) + Number(controls.steerRight) * -1

    const steering = MathUtils.lerp(currentSteering, steerAngle * steerDirection, 0.5)
  ...
  return (
    <>
      ...
      >
        ...
        <object3D position={[0.8, 0.3, -0.55]} ref={(ref: Object3D) => (wheelsRef.current[0] = ref)}>
          <WheelFront rotation={[0, Math.PI * -0.5, 0]} />
        </object3D>

        <object3D position={[0.8, 0.3, 0.55]} ref={(ref: Object3D) => (wheelsRef.current[1] = ref)}>
          <WheelFront rotation={[0, Math.PI * 0.5, 0]} />
        </object3D>

        <object3D position={[-0.4, 0.4, -0.75]} ref={(ref: Object3D) => (wheelsRef.current[2] = ref)}>
          <WheelBack rotation={[0, Math.PI * -0.5, 0]} />
        </object3D>

        <object3D position={[-0.4, 0.4, 0.75]} ref={(ref: Object3D) => (wheelsRef.current[3] = ref)}>
          <WheelBack rotation={[0, Math.PI * 0.5, 0]} />
        </object3D>
      </RigidBody>
    </>
